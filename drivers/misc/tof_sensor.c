	...
	unsigned long event, void *data)
{
	...
	if (event & REGULATOR_EVENT_DISABLE) {
		...
		if (tof_data->info.vdd_supply_enable && tof_data->avdd) {
		...
		if (tof_data->info.gpio_avdd_enable && tof_data->gpio_avdd) {
			rc = regulator_disable(tof_data->gpio_avdd);
			if (!rc)
				LOGD(&tof_data->client->dev,
					"%s: Regulator gpio_avdd disable OK",
					__func__);
			else
				LOGE(&tof_data->client->dev,
...
static int tof_sensor_power_init(struct tof_sensor_data *data)
{
	...
	if ((data->gpio_avdd == NULL) && data->info.gpio_avdd_enable) {
		data->gpio_avdd = regulator_get(&data->client->dev, "tof_gpio_avdd");
	...
	if ((data->vio == NULL) && data->info.vio_supply_enable) {
...
static int tof_sensor_power_deinit(struct tof_sensor_data *data)
{
	...
	if (data->info.vio_supply_enable && data->vio)
...
static int tof_sensor_parse_dt(struct tof_sensor_data *data)
{
	...
	rc = of_property_read_u32(data->client->dev.of_node,
		"sony,tof-vio-supply", &val_u32);
	if (rc < 0) {
		LOGE(&data->client->dev, "%s failed %d\n", __func__, __LINE__);
		goto fail;
	}
	data->info.vio_supply_enable = val_u32;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value == TOF_SENSOR_POWER_CONTROL_ON) {
		if (!data->ref_cnt) {
			...
			if (!rc && data->info.vio_supply_enable) {
		...
		if (data->ref_cnt > 0) {
			...
			if (!data->ref_cnt) {
				...
				if (data->info.vio_supply_enable) {
					rc = regulator_disable(data->vio);
					if (rc) {
						LOGE(&data->client->dev,
							"%s: Regulator vio disable failed ",
							__func__);
					} else {
						LOGE(&data->client->dev,
							"%s: power up vio disable OK",
							__func__);
					}
				}
